...
const CITIES = [
  { value: 'Tijuana', label: 'Tijuana, B.C.' },
  { value: 'Mexicali', label: 'Mexicali, B.C.' },
  { value: 'La Paz', label: 'La Paz, B.C.S.' },
  { value: 'Ensenada', label: 'Ensenada, B.C.' },
];
  ...
  return (
    <form className={`landing-search ${compact ? 'landing-search--compact' : ''}`} onSubmit={onSubmit} role="search">
      <label className="landing-search__location">
        <YommigoIcon name="ubicacion" size={110} alt="" loading="eager" />
      ...
      {!compact && <button type="submit" className="landing-search__button"><YommigoIcon name="busqueda" size={110} alt="" loading="eager" /><span>Buscar</span></button>}
    </form>
  );
}